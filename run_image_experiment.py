...
# learn distance without normalize eps
...
def get_args():
    ...
    parser.add_argument("--config", type=str, default='cifar10',choices=['cifar10','imagenet','celeba_hq','celeba'])
    parser.add_argument("--config_path", type=str, default=None) #None cifar10_adm celeba_ddim
    parser.add_argument("--norm_eps", type=int, default=1)
    parser.add_argument("--sampler", type=str, default='ddim',choices=['ddpm', 'ddim', 'ge','ddim_simple'])
    parser.add_argument("--num_timesteps", type=int, default=10)
    parser.add_argument("--max_T", type=int, default=990) # 960
    parser.add_argument("--start_sigma", type=float, default=80)
    parser.add_argument("--end_sigma", type=float, default=0.0)
    parser.add_argument("--sigma_style", type=str, default='Linear',choices=['Linear','DDIM','Scaled'])
    parser.add_argument("--sampler_var", type=str, default='fixedsmall', choices=['learned','fixedsmall','fixedlarge','none'])
    parser.add_argument("--eta", type=float, default=0.0)
    parser.add_argument("--refine_sigma", type=int, default=0)
    parser.add_argument("--continuous_t", type=int, default=0)
    ...
    parser.add_argument("--sigma_block", type=int, default=3)
    ...
    parser.add_argument("--ema_rate", type=float, default=0.9995)
    parser.add_argument("--batch_size", type=int, default=64) # cifar 10:128, imagenet: 64
    parser.add_argument("--microbatch", type=int, default=64) # cifar 10:128, imagenet: 16, celeba-hq 32, celeba 64
    ...
    parser.add_argument("--n_dp", type=int, default=2)
    ...
    #args.sampler_var = new_config.model.var_type

    def get_default(args):
        if args.config == 'cifar10':
            args.load_eps = 'store/models/ADM_cifar10_baseline.pt' # 'store/models/ADM_IP_015.pt'
            args.fid_target = 'store/fid/fid_cifar10_train.npz'
            args.norm_max =54.70 #54.63
            args.norm_min = 0.0
            #args.n_train_samples = 50000
            #args.sigma_block = 2
        elif args.config == 'imagenet':
            args.load_eps = 'store/models/256x256_diffusion_uncond.pt'
            args.fid_target = 'store/fid/fid_imagenet.npz'
            args.norm_max = 440.0
            args.norm_min = 0.0
            #args.n_train_samples = 1281167
            #args.sigma_block = 3
        elif args.config == 'celeba_hq':
            args.load_eps = 'store/models/celeba_hq.ckpt'
            #args.fid_target = 'store/fid/fid_celeba_hq_256x256.npz'
            args.fid_target = 'store/fid/fid_celebahq_1k_orig.npz'
            args.norm_max = 397.0
            args.norm_min = 0.0
        elif args.config == 'celeba':
            args.load_eps = 'store/models/celeba64_ckpt.pth'  # 'store/models/DDPM_IP_celeba64.pt'
            args.fid_target = 'store/fid/fid_celeba_train.npz'
...
def main(args, config):
    ...
    data = load_data(data_dir=data_config.data_dir, batch_size=args.batch_size, image_size=data_config.image_size, class_cond=False,
    ...
    ckpt_file = args.load_eps
    ckpt = dist_util.load_state_dict(
                ckpt_file, map_location='cpu'
            )
    if isinstance(ckpt, list):
        model = torch.nn.DataParallel(model)
        model.load_state_dict(ckpt[0], strict=True)
        ema_state = ckpt[-1]
        model = model.module
        for name, param in model.named_parameters():
            if param.requires_grad:
                param.data.copy_(ema_state[name].data)
    #ckpt = torch.load(ckpt_file, map_location='cpu')
    else:
        model.load_state_dict(ckpt)

    print('load eps model from', ckpt_file)
    model.eval()
    ...
    sampler = get_sampler(args.sampler, samp_config.num_diffusion_timesteps, args.num_timesteps,
                         ...
                         sampler_var=args.sampler_var, continuous_t=args.continuous_t, eta=args.eta, norm_eps=True)
    ...
    experiment.set_optimizers(args.lr, args.sigma_loss,
                              ...
                              resume_ema_model=args.resume_ema_model, resume_optim=args.resume_optim)
    ...
    experiment.train(epoch=args.epoch,iter_per_epoch=args.iter_per_epoch, eval_per_epoch=args.eval_per_epoch, save_per_epoch=args.save_per_epoch,
                     max_T=args.max_T, microbatch=args.microbatch if args.microbatch>0 else args.batch_size)
    print('train done')
    torch.cuda.empty_cache()
