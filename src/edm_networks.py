...
class SongUNet(torch.nn.Module):
    ...
    def encode(self, x, noise_labels, class_labels, augment_labels=None):
        # Mapping.
        emb = self.map_noise(noise_labels)
        emb = emb.reshape(emb.shape[0], 2, -1).flip(1).reshape(*emb.shape)  # swap sin/cos
        if self.map_label is not None:
...
class PureUNetBlock(torch.nn.Module):
    def __init__(self,
...
class SigmaModel(torch.nn.Module):
    def __init__(self, dim=4, channels=64, n_blocks=2, out_dim=1,dropout=0.1,
        ...
        self.down_layer = torch.nn.ModuleList([])
        inp_dim=dim
        for i in range(n_blocks):
            if inp_dim%2!=0:
                self.down_layer.append(torch.nn.ConstantPad2d((0, 1, 0, 1), 0))
                inp_dim +=1
            else:
                self.down_layer.append(torch.nn.Identity())
            res_nn = PureUNetBlock(in_channels=channels,out_channels=channels, attention= i%2==0 ,**block_kwargs)
            self.down_layer.append(res_nn)
            down_nn = Downsample(channels, True)
        ...
        self.fc_layer= torch.nn.ModuleList([
            ...
            torch.nn.SiLU()])
        self.final_mlp = torch.nn.Linear(fc_dim, out_dim)
...
class EDMHelper(torch.nn.Module):
    def __init__(self,
                 img_resolution,  # Image resolution.
                 img_channels,  # Number of color channels.
                 label_dim=0,  # Number of class labels, 0 = unconditional.
                 ...
                 sigma_max=float('inf'),  # Maximum supported noise level.
                 sigma_data=0.5,  # Expected standard deviation of the training data.
                 model=None,  # Keyword arguments for the underlying model.
                 ):
        super().__init__()
        self.img_resolution = img_resolution
        self.img_channels = img_channels
    ...
    def encode(self, x, sigma, class_labels=None, force_fp32=False, **model_kwargs):
        x = x.to(torch.float32)
        sigma = sigma.to(torch.float32).reshape(-1, 1, 1, 1)
        class_labels = None if self.label_dim == 0 else torch.zeros([1, self.label_dim],
                                                                    device=x.device) if class_labels is None else class_labels.to(
        ...
        feat = self.model.encode((c_in * x).to(dtype), c_noise.flatten(), class_labels=class_labels, **model_kwargs)
        return feat
